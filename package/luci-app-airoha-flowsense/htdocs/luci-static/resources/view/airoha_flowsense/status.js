...
.ppe-terminal{background:#0c0c0c;border:1px solid #2a2a2a;border-radius:6px;overflow:hidden;display:flex;flex-direction:column;max-width:100%;min-width:0}\
...
.ppe-terminal-body{padding:8px 10px;overflow:auto;flex:1;font-size:10px;line-height:1.55;color:#ccc;font-family:"Courier New",Courier,monospace;white-space:pre;word-break:normal;min-height:200px;max-height:320px}\
...
function renderCompassCards(cs, bypass, jitter, wan, wifi, bridge, mode) {
	...
	var latSub   = 'Jitter: '+(jitter.jitter||0).toFixed(1)+'ms  |  '+(jitter.samples||0)+' samples  |  Ping: '+latTarget+' ✏';
	...
	var cards = E('div', { 'class': 'compass-cards', 'id': 'compass-cards' }, [
	...
	attachPingTargetEditor(cards.querySelectorAll('.compass-card-sub')[2], latTarget);
...
function attachPingTargetEditor(element, target) {
	...
	element.onclick = function() {
		var newTarget = window.prompt(_('Ping target IP:'), target);
		...
		callSetPingTarget(newTarget).then(function(res) {
			if (res && res.success) {
				...
				window.alert(res && res.error ? res.error : _('Failed to set ping target'));
			}
		}).catch(function(err) {
			window.alert(_('Error: ') + err.message);
		});
	};
}
...
function updateCompassCards(cs, bypass, jitter, wan, wifi, bridge, mode) {
	...
	attachPingTargetEditor(divs[2] && divs[2].querySelector('.compass-card-sub'), latTarget);
...
function fsTerminal(ppe) {
	return E('div', { 'class': 'fs-terminal-wrap' }, renderPpeTerminal(ppe));
}
	...
	render: function(data) {
		...
		poll.add(L.bind(function() {
			return callGetOverview().then(function(overview) {
				...
				var oldTerminal = document.getElementById('ppe-terminal-body');
				var scrollTop = oldTerminal ? oldTerminal.scrollTop : 0;
				var scrollLeft = oldTerminal ? oldTerminal.scrollLeft : 0;
				var selection = window.getSelection ? window.getSelection() : null;
				var terminalSelected = !!(oldTerminal && selection && !selection.isCollapsed &&
					(oldTerminal.contains(selection.anchorNode) || oldTerminal.contains(selection.focusNode)));
				var ethPorts = (eth && Array.isArray(eth.ports)) ? eth.ports : [];
				var ethRates = fsSampleEthRates(ethPorts);
				var live = document.getElementById('flowsense-live');
				if (live) {
					...
					if (!editing) {
						...
						live.appendChild(fsDashboard(st, ppe, ti, fe, dm, bypass, wan, jitter,
				...
				if (oldTerminal && !terminalSelected) {
					oldTerminal.innerHTML = buildPpeTerminalBody(ppe);
					oldTerminal.scrollTop = scrollTop;
					oldTerminal.scrollLeft = scrollLeft;
				}
			},this));
		},this), 5);
